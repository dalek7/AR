 ...
 *  MLTest
 *
 *  Created by Seung-Chan Kim on 10/19/11.
...
inline void DrawArrow(IplImage* img, CPointf stPt, CPointf endPt, float scale_arrow,  CvScalar clr = CV_RGB(255,0,255), float startCircleSz=2)
{
...
// Col4x4toRow3x3(m1.v, H2->data.fl);
// dhMat to CvMat*
inline void Col4x4toRow3x3(float* f_col4, float* f_row3)
{
...
inline void PrintString3x3CvMat(CvMat *M, int row, char* str_dest)
{
	sprintf(str_dest, "%.3f %.3f %.3f", M->data.fl[3*row+ 0], M->data.fl[3*row+ 1], M->data.fl[3*row+ 2]);
}
...
inline void Print3x3CvMat(CvMat *M)
{
	...
	{
		printf("%.3f %.3f %.3f", M->data.fl[3*i+ 0], M->data.fl[3*i+ 1], M->data.fl[3*i+ 2]);
...
inline void Print3x3Mat(const cv::Mat& mat)
{
	...
	{
		printf("%.4f %.4f %.4f", mat.at<double>(3*i,1),mat.at<double>(3*i,2),mat.at<double>(3*i,3));
