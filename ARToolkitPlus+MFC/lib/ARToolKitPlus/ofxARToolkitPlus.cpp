...
void ofxARToolkitPlus::GetProjectionMatrix(float* v)
{
	const ARFloat *f = tracker->getProjectionMatrix();
	for(int i=0; i<16; i++)
	{
		v[i] = f[i];
	}
...
ofxARToolkitPlus::ofxARToolkitPlus() {
	...
	sf = 1;
	markerpos[0] = markerpos[1] = 0;
	//pTracker = (ARToolKitPlus::TrackerMultiMarker *) tracker;
}
...
//    tracker->setLogger(&logger);
	...
	string tmp1 = camParamFile;//ofToDataPath(camParamFile);
...
void ofxARToolkitPlus::draw(int x, int y, int w, int h) {
	...
	for(int i=0; i<numDetected; i++) 
	{
		...
		//ofFill();
		glColor3f(255, 0, 255 );//ofSetColor(255, 0, 255 );
		
		//ofRect(marker.pos[0]-1, height- marker.pos[1]-1, 2, 2);
		
		markerpos[0] = marker.pos[0];
		markerpos[1] = marker.pos[1];

		
		glBegin(GL_LINE_LOOP);//start drawing a line loop
			glVertex3f(marker.vertex[0][0], (height-marker.vertex[0][1]),0.0f);//left of window
			glVertex3f(marker.vertex[1][0], (height-marker.vertex[1][1]),0.0f);//bottom of window
			glVertex3f(marker.vertex[2][0], (height-marker.vertex[2][1]),0.0f);//right of window
			glVertex3f(marker.vertex[3][0], (height-marker.vertex[3][1]),0.0f);//top of window
...
void ofxARToolkitPlus::GetModelMatrix(int markerIndex, float* v)
{
...
void ofxARToolkitPlus::getMultiMarkerTranslationAndOrientation(ofVec3f &translation, ofMatrix4x4 &orientation) {
	...
	if(multiMarkerConst != NULL) {
		...
		if(result < 0 || result >= INT_MAX) {
			...
			printf("RPP failed on multimarker");
		} 
...
void ofxARToolkitPlus::getTransMat(ARToolKitPlus::ARMarkerInfo *marker_info, float center[2], float conv[3][4]) {
	...
	if(result < 0 || result >= INT_MAX) {
		...
		printf("RPP failed on marker %d", marker_info->id);
